...
def get_sde_loss_fn(sde, train, reduce_mean=True, continuous=True, likelihood_weighting=True, eps=1e-5):
  ...
  def loss_fn(model, batch, frame_no=None):
    ...
    if isinstance(batch, tuple):
      mag_batch, phase_batch = batch
      img_batch = phase_batch
    else:
      img_batch = batch
    score_fn = mutils.get_score_fn(sde, model, train=train, continuous=continuous)
    t = torch.rand(img_batch.shape[0], device=img_batch.device) * (sde.T - eps) + eps
    z = torch.randn_like(img_batch)
    mean, std = sde.marginal_prob(img_batch, t)
    perturbed_data = mean + std[:, None, None, None] * z

    if isinstance(batch, tuple):
      perturbed_data = torch.cat([perturbed_data, mag_batch], dim=1)
    score = score_fn(perturbed_data, t, frame_no)

    if not likelihood_weighting:
      losses = torch.square(score * std[:, None, None, None] + z)
      losses = reduce_op(losses.reshape(losses.shape[0], -1), dim=-1)
    else:
      g2 = sde.sde(torch.zeros_like(img_batch), t)[1] ** 2
...
def get_step_fn(sde, train, optimize_fn=None, reduce_mean=False, continuous=True, likelihood_weighting=False):
  ...
  def step_fn(state, batch, frame_no=None):
    ...
    if train:
      ...
      optimizer.zero_grad()
      loss = loss_fn(model, batch, frame_no)
      loss.backward()
