...
def trim_and_process(ind1a, ind1b, IMAGEN, frame_no):
    sz = IMAGEN.shape
    tmp = IMAGEN[ind1a-1:ind1b, :, :].reshape((ind1b - ind1a + 1, sz[1] * sz[2]))
    TRD_C = np.zeros((frame_no, sz[1] * sz[2]))

    for j in range(sz[1] * sz[2]):
        x = np.arange(len(tmp[:, j]))
        pchip_interpolator = PchipInterpolator(x, tmp[:, j])
        tmp1 = pchip_interpolator(np.linspace(0, len(tmp[:, j]) - 1, frame_no))
        TRD_C[:, j] = tmp1

    img = TRD_C.reshape((frame_no, sz[1], sz[2]))
    return img

## Helper functions 
def generate_dataset(data_path, folder_name="singlecoil_kspace_val", dataset="fastmri"):
